...
J = sum( sum( (-y1).*log(a3) - (1-y1).*log(1-a3)) )/m;

J = J + (lambda/(2*m))*sum(sum(Theta1(:,2:end).^2)) + (lambda/(2*m))*sum(sum(Theta2(:,2:end).^2));
...
% delta_2 = delta_3*Theta2(:,2:end).* sigmoidGradient(z2(i,:));
